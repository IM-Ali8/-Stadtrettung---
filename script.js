...
(function bindDesktopButtons(){
  ...
  if(musicBtn){
    musicBtn.addEventListener("click", (e)=>{
      e.preventDefault();
      e.stopPropagation();
      // اگر toggleMusic داری، همین
      if(typeof toggleMusic === "function") toggleMusic();
      else {
        // fallback ساده اگر تابعش را حذف کرده باشی
        audioOn = !audioOn;
        musicBtn.textContent = "Music: " + (audioOn ? "On" : "Off");
...
(function setupMobileControls(){
  ...
  const btnUp    = q("btnJet");    // ▲ jump
  const btnX     = q("btnJump");   // X shoot
  const btnJet   = q("btnShoot");  // JET jetpack
  const btnKey   = q("btnE");      // 🔑
  const btnStart = q("btnStart");  // mobile Start (داخل leftpad یا هرجا هست)
  const btnReset = q("btnReset");  // mobile Reset

  // اگر دکمه‌ای نبود، ساکت برگرد
  ...
  function press(code){
    ...
    if(code==="KeyX"){ shoot(); } // شلیک لحظه‌ای
  }
  ...
  function bindHold(el, code){
    ...
    const down = (e)=>{
      e.preventDefault();
      e.stopPropagation();
      // برای اینکه با کشیدن انگشت قطع نشه:
...
bindHold(btnRight, "ArrowRight");
  ...
  // X = shoot (tap/hold doesn't matter چون shoot خودش cooldown داره)
  bindHold(btnX, "KeyX");
...
function updateHud(){
  ...
  const keyEl = $("key"); 
  if(keyEl) keyEl.textContent = hasKey ? "✔" : "✖";

  const tgt = $("targetFa"); 
  if(tgt) tgt.textContent = currentTarget ? currentTarget.fa : "—";
...
buildDecks();
currentTarget = null; // تا قبل از Start چیزی نمایش نده
...
function shoot(){
  ...
  hero._shootCd = 0.14;
  hero._muzzleT = 0.06; // فلش شلیک کوتاه
  const dir = hero.face;
  bullets.push({
...
function update(dt){
 ...
 const accel = 2300;   // واکنش سریع‌تر
const maxV = 380;     // کمی سریع‌تر حرکت کنه
  let ax = 0;
...
function drawCage(){
  ...
  if(cage.open){
    ...
    ctx.fillText("🔒", cage.x + cage.w-17, cage.y + cage.h/2);
  }
}
function drawPrincess(){
  ...
  // ---- Body (مینیمال مثل عکس) ----
  ctx.fillStyle = "rgba(210,110,140,.95)"; // صورتی/زرشکی
  ctx.fillRect(px, py, w, h);

  // ---- Top band (نوار زرد بالا) ----
...
(function emergencyCapture(){
  ...
  // capture phase: قبل از هر overlay/stopPropagation
  document.addEventListener("pointerdown", handle, true);
  document.addEventListener("click", handle, true);

  log("Emergency capture installed ✅");
})();
function bindTapSingle(el, fn){
  ...
  const fire = (e)=>{
    ...
    lock = true;
    fn();
    setTimeout(()=>{ lock = false; }, 350); // ضد دوبار فایر شدن
  };

  // فقط یکی! (pointerup) — نه pointerdown، نه click
